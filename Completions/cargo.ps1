...
Register-ArgumentCompleter -Native -CommandName 'cargo' -ScriptBlock {
    ...
    $_cmds = (cargo --list | Select-Object -Skip 1 | ForEach-Object {
            $_cmd = ''
            $_rst = ''
            $_trm = $_.Trim()
            $_sp = $_trm.IndexOf([char]32)
            if ($_sp -eq -1) {
                $_cmd = $_trm.Trim()
            }
            else {
                $_cmd = $_trm.SubString(0, $_sp).Trim()
                $_rst = $_trm.SubString($_sp).Trim()
            }
            @{
                Original    = $_;
                Command     = $_cmd;
    ...
    [System.Management.Automation.Compl]::new()
